function AudioSynthView() {
    ...
    var fnChangeOctave = function(x) {
        ...
        select_base += x;
   		select_base = Math.max(0, Math.min(2, select_base));
    ...
    var fnCreateKeyboard = function(keyboardElement) {
        ...
        var iKeys = 0;
        var iWhite = 0;
        var notes = {'C':261.63,'C#':277.18,'D':293.66,'D#':311.13,'E':329.63,'F':346.23,'F#':369.99,'G':392.00,'G#':415.30,'A':440.00,'A#':466.16,'B':493.88};
        for (var i = -1; i <= 1; i++) {
            for (var n in notes) {
                if (n[2] != 'b') {
    ...
    window.addEventListener('keypress', function(e) {
            currentInstrument.handleKey(String.fromCharCode(e.keyCode));
    });
